...
contract FTO is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256[] private _rotationLength;
    string[] private _specieSelection;
   ...
   enum VineyardStatus {
    ...
    mapping(uint256 => VineyardStatus) private _vineyardStatus;
    ...
    event TokenMetadataUpdated(
        ...
        uint256 indexed rotationLength,
        string specieSelection,
        string provenance
    );
    ...
    constructor(address fftContractAddress, uint256 mintFee) ERC721("Fruittex Vineyards", "FTV") {
 ...
 function setVineyardStatus(uint256 tokenId, VineyardStatus status) external onlyOwner {
    require(_exists(tokenId), "FTO: Token does not exist");

    _vineyardStatus[tokenId] = status;

 
    if (status == VineyardStatus.Retired) {
        _pauseToken(tokenId);
    } else {
    ...
    function getFTODetails(uint256 tokenId) public view returns (
        ...
        rotationLength = _rotationLength[tokenId - 1];
        specieSelection = _specieSelection[tokenId - 1];
        provenance = _provenance[tokenId - 1];
   ...
   function createToken(
        ...
        _rotationLength.push(rotationLength);
        _specieSelection.push(specieSelection);
        _provenance.push(provenance);
        _term.push(term);
...
function calculateTotalTreesForAllFTOs() public view returns (uint256) {
 ...
 function calculateTotalFTOTreesByOwner(address owner) public view returns (uint256) {
    ...
    delete _rotationLength[tokenId];
    delete _specieSelection[tokenId];
    delete _provenance[tokenId];
    ...
    function updateTokenMetadata(
        ...
        _rotationLength[tokenId] = rotationLength;
        _specieSelection[tokenId] = specieSelection;
...
function getTokenInfo(uint256 tokenId) public view returns (TokenInfo memory) {
    ...
    return TokenInfo({
        ...
        rotationLength: _rotationLength[index],
        specieSelection: _specieSelection[index],
